...
class DBHelper {
  ...
  CryptoUtil _Encryptor;

  DBHelper() {
    _Encryptor = CryptoUtil();
  }
  ...
  static Future<List<Account>> setAddressForAccounts(
    ...
    accounts.forEach((a) {
      a.address = CryptoUtil.seedToAddress({'seed': seed, 'index': a.index});
    });
    return accounts;
  }
  ...
  Future<Account> addAccount(String seed, {String nameBuilder}) async {
    ...
    await dbClient.transaction((Transaction txn) async {
      ...
      account = Account(
          ...
          lastAccess: 0,
          selected: false,
          address: await _Encryptor.seedToAddressInIsolate(seed, nextIndex));
      await txn.rawInsert(
  ...
  Future<Account> getSelectedAccount(String seed) async {
    ...
    String address =
        await _Encryptor.seedToAddressInIsolate(seed, list[0]["acct_index"]);
